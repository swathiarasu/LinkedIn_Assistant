...
export default function DraftsStep({ profile, topic, ideaSeed, drafts, setDrafts, onBack, onRestart }) {
  ...
  const [posting, setPosting] = useState({}) // { [i]: 'posting' | 'done' | 'error' }
  ...
  // Save to DB + open LinkedIn in one click
  const handlePostToLinkedIn = async (draft, i) => {
    ...
    if (user) {
      ...
      if (saveError) {
        ...
        // Don't block the user — still open LinkedIn even if save fails
      }
    }
